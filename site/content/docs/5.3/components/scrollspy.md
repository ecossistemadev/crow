...
layout: docs
title: Scrollspy
description: Automatically update Crow navigation or list group components based on scroll position to indicate which link is currently active in the viewport.
group: components
toc: true
---
...
document.querySelectorAll('#nav-tab>[data-bs-toggle="tab"]').forEach(el => {
  el.addEventListener('shown.bs.tab', () => {
    ...
    crow.ScrollSpy.getOrCreateInstance(scrollElem).refresh()
  })
})
```
...
To easily add scrollspy behavior to your topbar navigation, add `data-bs-spy="scroll"` to the element you want to spy on (most typically this would be the `<body>`). Then add the `data-bs-target` attribute with the `id` or class name of the parent element of any Crow `.nav` component.
...
```js
const scrollSpy = new crow.ScrollSpy(document.body, {
...
dataSpyList.forEach(dataSpyEl => {
  crow.ScrollSpy.getInstance(dataSpyEl).refresh()
})
```
