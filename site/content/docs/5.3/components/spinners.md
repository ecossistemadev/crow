...
layout: docs
title: Spinners
description: Indicate the loading state of a component or page with Crow spinners, built entirely with HTML, CSS, and no JavaScript.
group: components
toc: true
---
...
Spinners in Crow are built with `rem`s, `currentColor`, and `display: inline-flex`. This means they can easily be resized, recolored, and quickly aligned.
...
As part of Crow's evolving CSS variables approach, spinners now use local CSS variables on `.spinner-border` and `.spinner-grow` for enhanced real-time customization. Values for the CSS variables are set via Sass, so Sass customization is still supported, too.
