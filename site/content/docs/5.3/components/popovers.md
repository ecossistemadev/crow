...
layout: docs
title: Popovers
description: Documentation and examples for adding Crow popovers, like those found in iOS, to any element on your site.
group: components
toc: true
---
...
- Popovers rely on the third party library [Popper](https://popper.js.org/) for positioning. You must include [popper.min.js]({{< param "cdn.popper" >}}) before `crow.js`, or use one `crow.bundle.min.js` which contains Popper.
...
const popoverList = [...popoverTriggerList].map(popoverTriggerEl => new crow.Popover(popoverTriggerEl))
```
...
Four options are available: top, right, bottom, and left. Directions are mirrored when using Crow in RTL. Set `data-bs-placement` to change the direction.
...
```js
const popover = new crow.Popover('.example-popover', {
  container: 'body'
})
```
...
```js
const popover = new crow.Popover('.popover-dismiss', {
  trigger: 'focus'
})
```
...
As part of Crow’s evolving CSS variables approach, popovers now use local CSS variables on `.popover` for enhanced real-time customization. Values for the CSS variables are set via Sass, so Sass customization is still supported, too.
...
const popover = new crow.Popover(exampleEl, options)
```
...
```js
const popover = new crow.Popover(element, {
...
const popover = crow.Popover.getOrCreateInstance('#example') // Returns a Crow popover instance
