...
<div class="offcanvas offcanvas-start show" tabindex="-1" id="offcanvas" aria-labelledby="offcanvasLabel">
  ...
  <div class="offcanvas-body">
    Content for the offcanvas goes here. You can place just about any Crow component or custom elements here.
  </div>
</div>
{{< /example >}}
...
As part of Crow's evolving CSS variables approach, offcanvas now uses local CSS variables on `.offcanvas` for enhanced real-time customization. Values for the CSS variables are set via Sass, so Sass customization is still supported, too.
...
const offcanvasList = [...offcanvasElementList].map(offcanvasEl => new crow.Offcanvas(offcanvasEl))
```
...
```js
const bsOffcanvas = new crow.Offcanvas('#myOffcanvas')
```
...
Crow's offcanvas class exposes a few events for hooking into offcanvas functionality.
