...
```js
const carousel = new crow.Carousel('#myCarousel')
```
...
const carousel = new crow.Carousel(myCarouselElement, {
  interval: 2000,
  touch: false
})
```
...
| `getInstance` | Static method which allows you to get the carousel instance associated to a DOM element. You can use it like this: `crow.Carousel.getInstance(element)`. |
| `getOrCreateInstance` | Static method which returns a carousel instance associated to a DOM element, or creates a new one in case it wasn't initialized. You can use it like this: `crow.Carousel.getOrCreateInstance(element)`. |
...
Crow's carousel class exposes two events for hooking into carousel functionality. Both events have the following additional properties:
