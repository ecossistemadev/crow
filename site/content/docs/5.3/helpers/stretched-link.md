...
description: Make any HTML element or Crow component clickable by "stretching" a nested link via CSS.
group: helpers
---
...
Cards have `position: relative` by default in Crow, so in this case you can safely add the `.stretched-link` class to a link in the card without any other HTML changes.
