---
layout: docs
title: Crow and Webpack
description: The official guide for how to include and bundle Crow's CSS and JavaScript in your project using Webpack.
group: getting-started
toc: true
thumbnail: guides/crow-webpack@2x.png
---

<img class="mb-4 img-fluid rounded-3" srcset="/docs/{{< param docs_version >}}/assets/img/guides/crow-webpack.png, /docs/{{< param docs_version >}}/assets/img/guides/crow-webpack@2x.png 2x" src="/docs/{{< param docs_version >}}/assets/img/guides/crow-webpack.png" width="2000" height="1000" alt="">

{{< callout >}}
**Want to skip to the end?** Download the source code and working demo for this guide from the [ecossistemadev/examples repository](https://github.com/ecossistemadev/examples/tree/main/webpack). You can also [open the example in StackBlitz](https://stackblitz.com/github/ecossistemadev/examples/tree/main/webpack?file=index.html) for live editing.
{{< /callout >}}

## Setup

We're building a Webpack project with Crow from scratch, so there are some prerequisites and upfront steps before we can really get started. This guide requires you to have Node.js installed and some familiarity with the terminal.
...
4. **Install additional dependencies.** In addition to Webpack and Crow, we need a few more dependencies to properly import and bundle Crow's CSS and JS with Webpack. These include Sass, some loaders, and Autoprefixer.
...
2. **Next we fill in our `src/index.html`.** This is the HTML page Webpack will load in the browser to utilize the bundled CSS and JS we'll add to it in later steps. Before we can do that, we have to give it something to render and include the `output` JS from the previous step.
   ...
   <html lang="pt-BR">
     <head>
       ...
       <title>Crow w/ Webpack</title>
     </head>
     <body>
       <div class="container py-4 px-3 mx-auto">
         <h1>Hello, Crow and Webpack!</h1>
   ...
   We're including a little bit of Crow styling here with the `div class="container"` and `<button>` so that we see when Crow's CSS is loaded by Webpack.
...
In the next and final section to this guide, we'll set up the Webpack loaders and import all of Crow's CSS and JavaScript.

## Import Crow

Importing Crow into Webpack requires the loaders we installed in the first section. We've installed them with npm, but now Webpack needs to be configured to use them.
...
2. **Now, let's import Crow's CSS.** Add the following to `src/scss/styles.scss` to import all of Crow's source Sass.
...
3. **Next we load the CSS and import Crow's JavaScript.** Add the following to `src/js/main.js` to load the CSS and import all of Crow's JS. Popper will be imported automatically through Crow.
...
4. **And you're done! 🎉** With Crow's source Sass and JS fully loaded, your local development server should now look like this:

   <img class="img-fluid" src="/docs/{{< param docs_version >}}/assets/img/guides/webpack-dev-server-crow.png" alt="Webpack dev server running with Crow">

   Now you can start adding any Crow components you want to use. Be sure to [check out the complete Webpack example project](https://github.com/ecossistemadev/examples/tree/main/webpack) for how to include additional custom Sass and optimize your build by importing only the parts of Crow's CSS and JS that you need.

## Production optimizations

Depending on your setup, you may want to implement some additional security and speed optimizations useful for running the project in production. Note that these optimizations are not applied on [the Webpack example project](https://github.com/ecossistemadev/examples/tree/main/webpack) and are up to you to implement.
